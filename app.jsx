...
const DEFAULT_THRESHOLDS = {
  turnoverWarn: 15,     // 庫存週轉天數 - 警戒(橙)
  turnoverDanger: 20,   // 庫存週轉天數 - 危險(紅)
  staleDays: 45,        // 呆滯判定天數(超過N天未出貨)
  onTimeTarget: 98,     // 出貨時效達成率目標(%)
  diffRateWarn: 1.0,    // 盤差率警戒(%)
  diffRateDanger: 2.0,  // 盤差率危險(%)
  maxLoginAttempts: 5,  // 登入鎖定門檻
  lockoutMinutes: 15
};
...
function genSkuData() {
  const names = [
    "USB-C充電線", "藍牙耳機", "無線滑鼠", "螺絲起子組", "LED燈泡", "延長線",
    "礦泉水(箱)", "洋芋片", "即溶咖啡", "棉質T恤", "運動襪", "帆布包",
    "冷凍水餃", "鮮乳", "冷凍雞胸肉", "文件夾", "原子筆(盒)", "A4影印紙",
    "行動電源", "藍牙喇叭", "五金收納盒", "餅乾禮盒", "牛仔褲", "保溫瓶"
  ];
  return names.map((name, i) => {
    ...
    let status = "正常";
    if (currentStock <= safetyStock * 0.3) status = "缺貨";
    else if (daysSinceLastOut > DEFAULT_THRESHOLDS.staleDays || daysOfStock > 60) status = "呆滯";
    else if (currentStock <= safetyStock) status = "低庫存";

    return {
      id: `SKU${String(i + 1).padStart(4, "0")}`,
      name, category, warehouse,
      currentStock, safetyStock, avgDailyOutbound, daysOfStock,
      daysSinceLastOut, unitCost, status,
...
function genPickingData() {
  ...
  days.forEach((day) => {
    shifts.forEach((shift) => {
      ...
      rows.push({ day, shift, pickedQty, workHours, efficiency: Math.round(pickedQty / workHours) });
    });
  });
  return rows;
}
...
function ToastProvider({ children }) {
  ...
  return (
    <ToastContext.Provider value={pushToast}>
      {children}
      <div className="fixed top-4 right-4 z-[999] flex flex-col gap-2 w-80 max-w-[90vw]">
        {toasts.map(t => (
          <div key={t.id}
               className={`toast-enter ${t.leaving ? "toast-leave" : ""} pointer-events-auto rounded-lg shadow-lg border px-4 py-3 text-sm flex items-start gap-2 ${
                 t.type === "error" ? "bg-red-50 border-red-300 text-red-700" :
                 t.type === "warning" ? "bg-orange-50 border-orange-300 text-orange-700" :
...
function EmptyState({ text }) {
  return <div className="py-10 text-center text-slate-400 text-sm">🔍 {text || "查無資料"}</div>;
}

function ConfirmModal({ open, title, message, onConfirm, onCancel }) {
  if (!open) return null;
  return (
    <div className="fixed inset-0 bg-black/40 z-[998] flex items-center justify-center p-4">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl p-6 w-full max-w-sm">
        ...
        <p className="text-sm text-slate-500 dark:text-slate-300 mb-5">{message}</p>
        <div className="flex justify-end gap-2">
          <button onClick={onCancel} className="px-4 py-1.5 text-sm rounded-lg border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-200">取消</button>
          <button onClick={onConfirm} className="px-4 py-1.5 text-sm rounded-lg bg-red-600 text-white hover:bg-red-700">確認</button>
        </div>
      </div>
    </div>
  );
...
function TurnoverTrendChart({ data, thresholds }) {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: data.map(d =>
          d.turnoverDays >= thresholds.turnoverDanger ? "#dc2626" :
          d.turnoverDays >= thresholds.turnoverWarn ? "#f59e0b" : "#16a34a"),
        borderRadius: 6
      }]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { display: false }, tooltip: { callbacks: { label: c => `${c.parsed.y} 天` } } },
      scales: { y: { beginAtZero: true, title: { display: true, text: "天" } } }
    }
  }), [data, thresholds]);
  return <canvas ref={canvasRef} height="220" data-chart-name="庫存週轉趨勢"></canvas>;
}

function StatusDonutChart({ counts, onSliceClick, activeStatus }) {
  const canvasRef = useRef(null);
  const labels = Object.keys(counts);
  const values = Object.values(counts);
  const colors = { "正常": "#16a34a", "低庫存": "#3b82f6", "呆滯": "#f59e0b", "缺貨": "#dc2626" };
  const chartRef = useChart(canvasRef, () => ({
    ...
    options: {
      ...
      plugins: {
        ...
        tooltip: { callbacks: { label: c => { const t = values.reduce((a,b)=>a+b,0); return ` ${c.label}：${c.parsed} 筆（${t? Math.round(c.parsed/t*100):0}%）`; } } }
      },
  ...
  return (
    <div>
      <canvas ref={canvasRef} height="220" className="chart-clickable" data-chart-name="庫存狀態分布"></canvas>
      {activeStatus && (
        <p className="text-center text-xs text-blue-600 mt-2">
          已篩選：{activeStatus}
          <button onClick={() => onSliceClick(null)} className="ml-2 underline">清除篩選</button>
        </p>
      )}
    </div>
  );
...
function PickingEfficiencyChart({ data }) {
  const canvasRef = useRef(null);
  const days = [...new Set(data.map(d => d.day))];
  const shifts = ["早班", "中班", "晚班"];
  const shiftColors = { "早班": "#2563eb", "中班": "#0d9488", "晚班": "#7c3aed" };
  useChart(canvasRef, () => ({
    type: "bar",
    data: {
      labels: days,
    ...
    options: {
      ...
      scales: { y: { beginAtZero: true, title: { display: true, text: "單/小時" } } }
    }
  }), [data]);
  return <canvas ref={canvasRef} height="220" data-chart-name="各班次揀貨效率"></canvas>;
}

// ----------------------------------------------------------------------------
// 6. 互動式資料表（搜尋 / 排序 / 分頁）
// ----------------------------------------------------------------------------
function DataTable({ columns, rows, pageSize = 8, rowKey = "id", onRowStatusHighlight }) {
  ...
  return (
    <div>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 mb-3">
        ...
        />
        <span className="text-xs text-slate-400">共 {sorted.length} 筆</span>
      </div>
      ...
      <div className="overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-700">
        <table className="min-w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
            ...
            {pageRows.map(row => (
              <tr key={row[rowKey]} className="data-row">
                {columns.map(c => (
                  <td key={c.key} className="px-3 py-2 whitespace-nowrap text-slate-700 dark:text-slate-200">
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-end gap-2 mt-3 text-sm">
          <button disabled={page === 1} onClick={() => setPage(p => p - 1)}
                  className="px-3 py-1 rounded border border-slate-300 dark:border-slate-600 disabled:opacity-40">上一頁</button>
          <span className="text-slate-500">{page} / {totalPages}</span>
          <button disabled={page === totalPages} onClick={() => setPage(p => p + 1)}
                  className="px-3 py-1 rounded border border-slate-300 dark:border-slate-600 disabled:opacity-40">下一頁</button>
        </div>
      )}
    </div>
  );
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        if (!json.length) { reject(new Error("檔案內容為空，找不到任何資料列。")); return; }
        ...
        const rows = json.map((r, i) => {
          ...
          let status = "正常";
          if (currentStock <= safetyStock * 0.3) status = "缺貨";
          else if (daysOfStock > 60) status = "呆滯";
          else if (currentStock <= safetyStock) status = "低庫存";
          return {
            id: String(r["SKU編號"]), name: String(r["品名"]), category: String(r["分類"]),
            warehouse: String(r["倉別"]), currentStock, safetyStock, avgDailyOutbound,
            daysOfStock, daysSinceLastOut: 0, unitCost: Number(r["單位成本"]) || 0,
            status, updatedAt: new Date().toISOString().slice(0, 10)
          };
        }).filter(Boolean);
...
function ExcelUploadPanel({ onDataLoaded }) {
  ...
  async function handleFile(file) {
    ...
    setLoading(true);
    try {
      const { rows, warnings } = await parseSkuExcel(file);
      onDataLoaded(rows);
  ...
  return (
    <div>
      ...
      >
        ...
        {loading ? (
          <p className="text-sm text-blue-600">解析中，請稍候...</p>
        ) : (
          <>
            <p className="text-sm font-medium text-slate-600 dark:text-slate-300">📤 點擊或拖曳 Excel 檔案至此處上傳，重新繪製儀表板</p>
            <p className="mt-1 text-xs text-slate-400">必要欄位：SKU編號、品名、分類、倉別、目前庫存、安全庫存、日均出貨量</p>
          </>
        )}
      </div>
    </div>
...
function DashboardPage({ role, skuData, setSkuData, turnoverTrend, shipments, pickingData, countData, thresholds }) {
  ...
  return (
    <div id="dashboard-capture" className="space-y-6">
      {/* 篩選列 */}
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4 flex flex-col sm:flex-row sm:items-center gap-3 no-print">
        <span className="text-sm font-medium text-slate-500 dark:text-slate-300 shrink-0">篩選條件</span>
        ...
        {statusFilter && (
          <span className="text-xs bg-blue-50 text-blue-600 px-3 py-1.5 rounded-lg">
            狀態篩選：{statusFilter}
            <button onClick={() => setStatusFilter(null)} className="ml-2 underline">清除</button>
          </span>
        )}
        <div className="sm:ml-auto flex gap-2">
          <button onClick={() => exportDashboardAsPdf("dashboard-capture", toast)}
                  className="px-3 py-1.5 text-sm rounded-lg bg-slate-700 text-white hover:bg-slate-800">📄 匯出 PDF</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-200">庫存狀態分布</h3>
            <button onClick={() => exportChartAsPng("庫存狀態分布")} className="text-xs text-blue-600 hover:underline no-print">下載圖檔</button>
          </div>
          <div style={{ height: 240 }}>
            <StatusDonutChart counts={statusCounts} activeStatus={statusFilter} onSliceClick={setStatusFilter} />
          </div>
        </div>
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4 lg:col-span-3">
          <div className="flex items-center justify-between mb-2">
            <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-200">各班次揀貨效率（近7天）</h3>
            <button onClick={() => exportChartAsPng("各班次揀貨效率")} className="text-xs text-blue-600 hover:underline no-print">下載圖檔</button>
          </div>
          <div style={{ height: 240 }}><PickingEfficiencyChart data={pickingData} /></div>
        </div>
      </div>

      {/* 資料表 */}
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4">
        <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-200 mb-3">庫存明細（SKU 層級）</h3>
        <DataTable columns={skuColumns} rows={filteredSku} />
      </div>

      {/* Excel 上傳（僅管理員可上傳重繪） */}
      {role === "admin" && (
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4 no-print">
          <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-200 mb-3">Excel 上傳重繪</h3>
          <ExcelUploadPanel onDataLoaded={setSkuData} />
        </div>
      )}
    </div>
  );
...
function loadMockUsers() {
  ...
  return [
    { id: "admin", name: "系統管理員", role: "admin", warehouse: "全倉", locked: false, lastLogin: "2026-07-03 09:12" },
    { id: "viewer", name: "倉儲主管", role: "general", warehouse: "北區倉", locked: false, lastLogin: "2026-07-03 08:40" },
    { id: "chen01", name: "陳倉管", role: "general", warehouse: "中區倉", locked: true, lastLogin: "2026-06-28 17:05" },
    { id: "wu02", name: "吳採購", role: "general", warehouse: "南區倉", locked: false, lastLogin: "2026-07-02 14:22" }
  ];
}
...
function UserPermissionPage() {
  ...
  useEffect(() => { localStorage.setItem(MOCK_USERS_KEY, JSON.stringify(users)); }, [users]);
  ...
  const columns = [
    ...
    { key: "warehouse", label: "所屬倉別" },
    { key: "lastLogin", label: "最後登入時間" },
    {
      key: "locked", label: "帳號狀態",
      render: r => r.locked
        ? <span className="px-2 py-0.5 rounded-full text-xs font-semibold bg-red-100 text-red-700">已鎖定</span>
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-200">使用者權限管理</h3>
        <span className="text-xs text-slate-400">一般使用者僅能檢視儀表板；管理員可變更資料與進階設定</span>
      </div>
      <DataTable columns={columns} rows={users} pageSize={10} />
      <ConfirmModal
        open={!!confirmTarget}
        title={confirmTarget?.locked ? "解除鎖定確認" : "鎖定帳號確認"}
        message={`確定要${confirmTarget?.locked ? "解除鎖定" : "鎖定"}帳號「${confirmTarget?.id}」嗎？`}
        onConfirm={() => { toggleLock(confirmTarget.id); setConfirmTarget(null); }}
...
function loadDashboardCards() {
  ...
  if (saved) return JSON.parse(saved);
  return [
    { id: 1, name: "庫存週轉天數", source: "WMS 庫存主檔", refreshFreq: "每小時", visible: true, updatedAt: "2026-07-03 08:00" },
    { id: 2, name: "缺貨／呆滯預警", source: "WMS 庫存主檔", refreshFreq: "每小時", visible: true, updatedAt: "2026-07-03 08:00" },
    { id: 3, name: "出貨時效達成率", source: "TMS 出貨紀錄", refreshFreq: "每日", visible: true, updatedAt: "2026-07-03 06:00" },
    { id: 4, name: "揀貨效率", source: "WMS 揀貨紀錄", refreshFreq: "每日", visible: true, updatedAt: "2026-07-03 06:00" },
    { id: 5, name: "盤差率與損耗", source: "盤點作業紀錄", refreshFreq: "每週", visible: true, updatedAt: "2026-06-30 18:00" }
  ];
}
...
function DashboardSettingsPage() {
  ...
  useEffect(() => { localStorage.setItem(DASHBOARD_CARDS_KEY, JSON.stringify(cards)); }, [cards]);

  function toggleVisible(id) {
    setCards(prev => prev.map(c => c.id === id ? { ...c, visible: !c.visible } : c));
  }
  function saveEdit(updated) {
    setCards(prev => prev.map(c => c.id === updated.id ? { ...updated, updatedAt: new Date().toISOString().slice(0, 16).replace("T", " ") } : c));
  ...
  const columns = [
    ...
    { key: "refreshFreq", label: "更新頻率" },
    {
      key: "visible", label: "顯示於儀表板",
      render: r => (
        <button onClick={() => toggleVisible(r.id)}
                className={`w-11 h-6 rounded-full relative transition ${r.visible ? "bg-green-500" : "bg-slate-300"}`}>
          <span className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition ${r.visible ? "left-5" : "left-0.5"}`}></span>
        </button>
      )
    },
    { key: "updatedAt", label: "最後更新時間" },
    {
      ...
      render: r => (
        <div className="flex gap-2">
          <button onClick={() => setEditing(r)} className="text-xs px-3 py-1 rounded-lg bg-blue-600 hover:bg-blue-700 text-white">編輯</button>
          <button onClick={() => removeCard(r.id)} className="text-xs px-3 py-1 rounded-lg bg-slate-400 hover:bg-slate-500 text-white">刪除</button>
        </div>
      )
    }
  ];
  ...
  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4">
      <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-200 mb-3">物流儀表板設定</h3>
      <DataTable columns={columns} rows={cards} pageSize={10} rowKey="id" />

      {editing && (
        <div className="fixed inset-0 bg-black/40 z-[998] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-xl p-6 w-full max-w-sm space-y-3">
            <h3 className="font-bold text-slate-800 dark:text-white">編輯卡片設定</h3>
            <div>
            ...
            <div>
              ...
              <select value={editing.refreshFreq} onChange={e => setEditing({ ...editing, refreshFreq: e.target.value })}
                      className="w-full mt-1 px-3 py-1.5 text-sm border border-slate-300 dark:border-slate-600 dark:bg-slate-700 rounded-lg">
                <option>即時</option><option>每小時</option><option>每日</option><option>每週</option>
              </select>
            </div>
            <div className="flex justify-end gap-2 pt-2">
              <button onClick={() => setEditing(null)} className="px-4 py-1.5 text-sm rounded-lg border border-slate-300 dark:border-slate-600">取消</button>
              <button onClick={() => saveEdit(editing)} className="px-4 py-1.5 text-sm rounded-lg bg-blue-600 text-white hover:bg-blue-700">儲存</button>
            </div>
          </div>
        </div>
      )}
...
function SystemSettingsPage({ thresholds, setThresholds }) {
  ...
  const fields = [
    { key: "turnoverWarn", label: "庫存週轉天數－警戒值（天）" },
    { key: "turnoverDanger", label: "庫存週轉天數－危險值（天）" },
    { key: "staleDays", label: "呆滯判定天數（天）" },
    { key: "onTimeTarget", label: "出貨時效達成率目標（%）" },
    { key: "diffRateWarn", label: "盤差率警戒值（%）" },
    { key: "diffRateDanger", label: "盤差率危險值（%）" },
    { key: "maxLoginAttempts", label: "登入錯誤鎖定次數" },
    { key: "lockoutMinutes", label: "帳號鎖定時間（分鐘）" }
  ];

  return (
    <div className="bg-white dark:bg-slate-800 rounded-xl shadow p-4 max-w-2xl">
      <h3 className="text-sm font-semibold text-slate-600 dark:text-slate-200 mb-4">系統設定 — 業務門檻與登入安全</h3>
      ...
      <button onClick={save} className="mt-5 px-5 py-2 text-sm rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium">儲存設定</button>
    </div>
  );
}
...
// ----------------------------------------------------------------------------
const NAV_ITEMS = [
  { key: "dashboard", label: "儀表板首頁", icon: "📊", roles: ["admin", "general"] },
  { key: "users", label: "使用者權限管理", icon: "👥", roles: ["admin"] },
  { key: "cards", label: "物流儀表板設定", icon: "⚙️", roles: ["admin"] },
  { key: "settings", label: "系統設定", icon: "🛠️", roles: ["admin"] }
];

function Sidebar({ role, active, onSelect, mobileOpen, onCloseMobile }) {
    ...
    <>
      ...
      <aside className={`fixed lg:static top-0 left-0 h-full w-64 bg-[#0f1c3f] text-white z-40 transition-transform duration-200
        ...
        <div className="px-5 py-5 border-b border-white/10 flex items-center gap-2">
          ...
          <div>
            <p className="font-bold text-sm leading-tight">供應鏈物流戰情室</p>
            <p className="text-[10px] text-slate-400">War Room Console</p>
          </div>
        </div>
        <nav className="p-3 space-y-1">
          {items.map(item => (
            <button key={item.key}
                    onClick={() => { onSelect(item.key); onCloseMobile(); }}
                    className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition ${
                      active === item.key ? "bg-[#1c3a78] text-white" : "text-slate-300 hover:bg-white/5"
                    }`}>
...
function Topbar({ session, darkMode, onToggleDark, onLogout, onOpenMobile }) {
  ...
  return (
    <header className="bg-white dark:bg-slate-800 shadow-sm px-4 py-3 flex items-center justify-between sticky top-0 z-20 no-print">
      <div className="flex items-center gap-3">
        <button onClick={onOpenMobile} className="lg:hidden text-xl">☰</button>
        <h1 className="text-base sm:text-lg font-bold text-slate-800 dark:text-white">供應鏈與倉儲物流戰情室</h1>
      </div>
      <div className="flex items-center gap-3 text-sm">
        <span className="hidden sm:inline text-slate-400 font-mono">{timeStr}</span>
        <button onClick={onToggleDark} className="px-2.5 py-1 rounded-lg border border-slate-300 dark:border-slate-600 text-xs">
          {darkMode ? "☀️ 淺色" : "🌙 深色"}
        </button>
        <span className="hidden sm:inline text-slate-500 dark:text-slate-300">
          {session.name}（{session.role === "admin" ? "管理員" : "一般使用者"}）
        </span>
        <button onClick={onLogout} className="px-3 py-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 text-xs font-medium">登出</button>
      </div>
    </header>
  );
}
...
function AppShell() {
  ...
  return (
    <div className="flex min-h-screen">
      <Sidebar role={session.role} active={active} onSelect={setActive} mobileOpen={mobileOpen} onCloseMobile={() => setMobileOpen(false)} />
